...
<?php
	...
	$tag->input('type="hidden" name="id" value="'.$contos->id.'" required aria-required="true"');
	$tag->div('class="col-sm-4"');
	    ...
	    $tag->div('class="form-group"');
	        $tag->div('class="form-line"');
	        	$tag->input('type="text" name="titulo" value="'.$contos->titulo.'" required aria-required="true" class="form-control" placeholder=""');
			$tag->div;
		$tag->div;
	$tag->div;
	...
	$tag->div('class="col-sm-4"');
	    ...
	    $tag->div('class="form-group"');
	        $tag->div('class="form-line"');
	        	$tag->input('type="text" name="tipo" value="'.$contos->tipo.'" required aria-required="true" class="form-control" placeholder=""');
			$tag->div;
		$tag->div;
	$tag->div;
	    ...
	    $tag->div('class="form-group"');
	        $tag->div('class="form-line"');
	        	$tag->textarea('class="form-control" rows="1" id="comment" name="descricao_pequena"');
	        		$tag->printer($contos->descricao_pequena);
	        	$tag->textarea;
			$tag->div;
		$tag->div;
	$tag->div;
