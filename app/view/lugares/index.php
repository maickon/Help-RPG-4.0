		                            ...
		                            $tag->tbody();
		                                foreach ($lugares as $key => $value) {
		                                    $tag->tr();
		                                    	$form->th($value->id);
		                                    	$form->th($value->dono);
												$form->th("<a href=\"".URL_BASE."lugares/visualizar/{$value->id}\">{$value->nome}</a>");
												$form->th($value->tipo);
		                                        ...
		                                        if ($value->dono == $_SESSION['login']) {
		                                            $tag->th('class="btns-del-edit"');
		                                                $tag->div('class="icon-button-demo js-modal-buttons"');
		                                                    $tag->button('data-color="red" class="icon-button-demo btn bg-red btn-xs waves-effect"');
		                                                        $tag->i('class="material-icons" onclick="deletar_url(\''.URL_BASE.'lugares/deletar/'.$value->id.'\')"');
		                                                            $tag->printer('delete');
		                                                        $tag->i;
		                                                    $tag->button;
		                                                $tag->div;
		                                                $tag->div('class="icon-button-demo"');
		                                                    $tag->a('href="'.URL_BASE.'lugares/editar/'.$value->id.'"');
		                                            ...
		                                            $tag->th('class="btns-del-edit"');
		                                                $tag->a('href="'.URL_BASE.'lugares/visualizar/'.$value->id.'" ');
