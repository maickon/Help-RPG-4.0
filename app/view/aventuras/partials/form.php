...
<?php
	...
	$tag->input('type="hidden" name="id" value="'.$aventuras->id.'" required aria-required="true"');
	$tag->div('class="col-sm-4"');
	    ...
	    $tag->div('class="form-group"');
	        $tag->div('class="form-line"');
	        	$tag->input('type="text" name="titulo" value="'.$aventuras->titulo.'" required aria-required="true" class="form-control" placeholder=""');
			$tag->div;
		$tag->div;
	$tag->div;
	$tag->div('class="col-md-4"');
	    ...
	    $tag->select('class="form-control show-tick" name="sistema" data-live-search="true" id="lista"');
	    	foreach ($sistemas_helper->rpg_nomes as $key => $value) {
	    		if (trim($value) == $aventuras->sistema) {
		            $tag->option('selected');
		            	$tag->printer($value);
		            $tag->option;
	    		} else {
	    ...
	    $tag->div('class="form-group"');
	        $tag->div('class="form-line"');
	        	$tag->textarea('class="form-control" rows="1" id="comment" name="descricao_pequena"');
	        		$tag->printer($aventuras->descricao_pequena);
	        	$tag->textarea;
			$tag->div;
		$tag->div;
	$tag->div;
	    ...
	    $tag->div('class="form-group"');
	        $tag->div('class="form-line"');
	        	$tag->textarea('id="ckeditor" name="descricao"');
	        		$tag->printer($aventuras->descricao);
	        	$tag->textarea;
			$tag->div;
		$tag->div;
	$tag->div;