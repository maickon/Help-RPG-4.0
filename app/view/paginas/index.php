		                            ...
		                            $tag->tbody();
		                                foreach ($paginas as $key => $value) {
		                                    $tag->tr();
		                                    	...
		                                    	$form->th($value->dono);
												$tag->th();
		                                    		$tag->a('href="'.URL_BASE.'paginas/visualizar/'.$value->id.'" ');
		                                            ...
		                                            $tag->th('class="btns-del-edit"');
		                                                $tag->div('class="icon-button-demo js-modal-buttons"');
		                                                    $tag->button('data-color="red" class="icon-button-demo btn bg-red btn-xs waves-effect"');
		                                                        $tag->i('class="material-icons" onclick="deletar_url(\''.URL_BASE.'paginas/deletar/'.$value->id.'\')"');
		                                                            $tag->printer('delete');
		                                                        $tag->i;
		                                                    $tag->button;
		                                                $tag->div;
		                                                $tag->div('class="icon-button-demo"');
		                                                    $tag->a('href="'.URL_BASE.'paginas/editar/'.$value->id.'"');
