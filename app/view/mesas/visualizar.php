	            ...
	            $tag->div('class="col-lg-12 col-md-12 col-sm-12 col-xs-12"');
	            	...
	            	$tag->div('class="card"');
	                    $tag->div('class="header"');
	                    	$tag->ul('class="header-dropdown m-r--5"');
	                            $tag->li('class="dropdown open"');
		                            ...
		                            $tag->ul('class="dropdown-menu pull-right"');
		                                ...
		                                $tag->printer('<li><a href="'.URL_BASE.'mesas" class=" waves-effect waves-block">'.$language->MENU_HOME.'</a></li>');
		                                $tag->printer('<li><a href="'.URL_BASE.'mesas/novo" class=" waves-effect waves-block">'.$language->BUTTON_SUBSCRIBE.'</a></li>');
		                                 if ($_SESSION['nome'] == $mesas_view->usuario_nome) {
		                                	$tag->printer('<li><a href="'.URL_BASE.'mesas/editar/'.$mesas_view->id.'" class=" waves-effect waves-block">'.$language->BUTTON_UPDATE.'</a></li>');
				    	                }
		                            $tag->ul;
	                            $tag->li;
	                        $tag->ul;
	                    	$tag->h2();
				                $tag->printer("
				                		<span class=\"font-bold col-blue\">{$mesas_view->nome}</span>
				                		{$language->SITE_SUBSCRIBE_BY}
				                		<span class=\"font-bold col-blue\">
				                			<a onmouseover=\"display_profile();\" onmouseout=\"hide_profile();\" href=\"".URL_BASE."usuario/profile/{$mesas_view->usuario_id}\" target=\"_blank\">
				                				{$mesas_view->usuario_nome}
				                			</a>
				                		</span>");
				            $tag->h2;
	                    $tag->div;
					                    ...
					                    $tag->div('class="body"');
					                    	$tag->div('class="row clearfix"');
						                    	$tag->div('class="col-md-2"');
		                    						$tag->img('src="'.$mesas_view->foto_link.'" class="img-responsive thumbnail"');
						                    	$tag->div;
						                    	$tag->div('class="col-md-3"');
						                    		$tag->b();
						                    			$tag->printer("{$language->USER_LABEL_NAME}");
						                    		$tag->b;
						                    		$tag->printer("{$mesas_view->usuario_nome}");
						                    	$tag->div;
						                    	...
						                    	$tag->div('class="col-md-3"');
						                    		...
						                    		$tag->b;
						                    		$tag->printer("{$mesas_view->estrelas}");
						                    	$tag->div;

						                    	$tag->div('class="col-md-10"');
						                    		$tag->b();
						                    			$tag->printer("{$language->USER_LABEL_ABOUT} {$mesas_view->usuario_nome}");
						                    		$tag->b;
						                    		$tag->printer("{$mesas_view->usuario_descricao}");
						                    	$tag->div;
	                    ...
	                    $tag->div('class="body"');
	                    	$tag->div('class="row clearfix"');
		                            ...
		                            $tag->div('class="col-sm-12"');
		                                ...
		                                $tag->div('class="form-group"');
		                                    $tag->div('class="form-line"');
				                                $tag->printer($mesas_view->sistema);
		                                    $tag->div;
		                                $tag->div;
		                            $tag->div;
		                            $tag->div('class="col-sm-12"');
		                                ...
		                                $tag->div('class="form-group"');
		                                    $tag->div('class="form-line"');
				                                $tag->printer(date('d/m/Y',strtotime($mesas_view->data_final)));
		                                    $tag->div;
		                                $tag->div;
		                            $tag->div;
		                            $tag->div('class="col-sm-12"');
		                                ...
		                                $tag->div('class="form-group"');
		                                    $tag->div('class="form-line"');
				                                $tag->printer($mesas_view->descricao);
		                                    $tag->div;
		                                $tag->div;
		                            $tag->div;
	                        $tag->div;
