...
<?php
	...
	$tag->input('type="hidden" name="id" value="'.$classes->id.'" required aria-required="true"');
	$tag->div('class="col-sm-4"');
	    ...
	    $tag->div('class="form-group"');
	        $tag->div('class="form-line"');
	        	$tag->input('type="text" name="nome" value="'.$classes->nome.'" required aria-required="true" class="form-control" placeholder=""');
			$tag->div;
		$tag->div;
	$tag->div;
	...
	$selects = ['Classe Básica','Classe de Prestígio'];
	...
	$tag->div('class="col-md-4"');
	    ...
	    $tag->select('class="form-control show-tick" name="tipo" data-live-search="true" id="tipos"');
