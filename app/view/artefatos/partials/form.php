...
<?php
	...
	$tag->input('type="hidden" name="id" value="'.$artefatos->id.'" required aria-required="true"');
	$tag->div('class="col-sm-4"');
	    ...
	    $tag->div('class="form-group"');
	        $tag->div('class="form-line"');
	        	$tag->input('type="text" name="nome" value="'.$artefatos->nome.'" required aria-required="true" class="form-control" placeholder=""');
			$tag->div;
		$tag->div;
	$tag->div;
	...
	$tag->div('class="col-md-4"');
	    ...
	    $tag->select('class="form-control show-tick" name="sistema" data-live-search="true" id="lista"');
	    	foreach ($sistemas_helper->rpg_nomes as $key => $value) {
	    		if (trim($value) == $artefatos->sistema) {
		            $tag->option('selected');
		            	$tag->printer($value);
		            $tag->option;
	    		} else {
	    ...
	    $tag->div('class="form-group"');
	        $tag->div('class="form-line"');
	        	$tag->textarea('id="ckeditor" name="descricao"');
	        		$tag->printer($artefatos->descricao);
	        	$tag->textarea;
			$tag->div;
		$tag->div;
	$tag->div;
