...
<?php
	...
	$tag->div('class="col-sm-4"');
	    ...
	    $tag->div('class="form-group"');
	        $tag->div('class="form-line"');
	        	$tag->input('type="text" name="titulo" value="'.$historias->titulo.'" required aria-required="true" class="form-control" placeholder=""');
			$tag->div;
		$tag->div;
	$tag->div;
	...
	$tag->div('class="col-sm-4"');
	    ...
	    $tag->div('class="form-group"');
	        $tag->div('class="form-line"');
	        	$tag->input('type="text" name="tipo" value="'.$historias->tipo.'" required aria-required="true" class="form-control" placeholder="romance, terror, meieval, futurista, etc..."');
			$tag->div;
		$tag->div;
	$tag->div;
	$tag->div('class="col-sm-12"');
	    ...
	    $tag->div('class="form-group"');
	        $tag->div('class="form-line"');
	        	$tag->textarea('class="form-control" rows="2" id="comment" name="descricao_pequena"');
	        		$tag->printer($historias->descricao_pequena);
	        	$tag->textarea;
			$tag->div;
		$tag->div;
	$tag->div;
