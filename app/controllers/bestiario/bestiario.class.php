...
<?php
	...
	class Bestiario_Controller extends Controller_Lib {
		...
		function index(){
			...
			$bestiario_model = new Bestiario_Model;
			...
			$bestiario = $bestiario_model->select('bestiario');
		...
		function salvar(){
			...
			if (count($_REQUEST) < 4) {
				...
				if ($bestiario->insert('bestiario', $campos, $valores)) {
					$time_line = new Timeline_Model;
					$id_bestiario = $bestiario->maxId('bestiario')[0];

					$time_line->insert(
					'timeline',
					['id_cadastro','lingua','titulo','exibir','tipo','dono','sistema','descricao'],
					[$id_bestiario->id,$_REQUEST['lingua'], $_REQUEST['nome'], $exibir, 'bestiario', $_REQUEST['dono'], $_REQUEST['sistema'], $_REQUEST['descricao']]);
					...
					header("Location: {$this->visualizar_path}{$id_bestiario->id}");
				} else {
		...
		function visualizar($params=''){
			...
			if ($params == '' || !is_numeric($params)) {
				header('Location:' . URL_BASE. 'bestiario');
			}
			...
			$bestiario_view = $bestiario->direct_instruction("SELECT a.*, {$usuario_atributos} FROM bestiario a, usuarios u WHERE a.id = {$params} and u.login = a.dono", 'bestiario e usuarios')[0];
		...
		function editar($params){
			$bestiario = new Bestiario_Model();
			$bestiario_view = $bestiario->direct_instruction("SELECT * FROM bestiario WHERE id={$params} AND dono='{$_SESSION['login']}'",'bestiario');

			if (is_numeric($params) and count($bestiario_view) == 1) {
				$bestiario = (new Bestiario_Model())->select('bestiario','*',[ ['id','=',$params] ]);
				if (count($bestiario) == 0) {
					header('Location: ' . URL_BASE . 'erro/codigo/404');
				} else {
					$bestiario = $bestiario[0];
					$language = new Locale_Lib;
					$tag = new Tags_Lib;
					$form = new Form_Lib;
					$painel = new Painel_Model;
		...
		function atualizar(){
			...
			header('Location: ' . URL_BASE . 'erro/codigo/404');
				...
				$bestiario = new Bestiario_Model();
				$bestiario_view = $bestiario->direct_instruction("SELECT * FROM bestiario WHERE id={$_REQUEST['id']} AND dono='{$_SESSION['login']}'",'armas');
				if (count($bestiario_view) == 1) {
					if( preg_match( '@<img(.*?)/>@s', $_REQUEST['descricao'], $matches ) ){
						$exibir = $matches[0];
					} else {
						$exibir = '';
					...
					if ($bestiario->update('bestiario', $campos, $valores, 'id', $_REQUEST['id'])) {
						...
						$insrucao = "
							...
							WHERE
								id_cadastro = {$_REQUEST['id']} AND tipo = 'bestiario'
						";

						$time_line->direct_instruction($insrucao,'timeline');

						header('Location: ' . URL_BASE . 'bestiario/visualizar/'.$_REQUEST['id']);
					} else {
		...
		function deletar($params = ''){
			...
			if ($params == '' || !is_numeric($params) || (count($bestiario_view) == 0)) {
				header('Location:' . URL_BASE. 'bestiario');
			} else {
				if ($bestiario->delete('bestiario', ['id','=', $params])){
					$time_line = new Timeline_Model;
					$time_line->delete('timeline', [ ['id_cadastro', '=', $params], ['tipo', '=', 'bestiario'] ], 'AND');
					header('Location: ' . URL_BASE . 'bestiario?status=deletado!');
				} else {
					header('Location: ' . URL_BASE . 'erro/codigo/003');
				}
			}
