...
Class Tavernas_Controller{
	...
	function medieval(){
		...
		$taverna_medieval = [
			...
			'taverneiro_nome' => $language->TAVERN_OWNER . $taverna->taverna_nome_taverneiro(),
			'taverneiro_raca' => $language->TAVERN_RACE . $raca,
			'taverneiro_idade' => $language->TAVERN_WITH . $idade_taverneiro . $language->TAVERN_YEARS,
			'taverneiro_profissao' => $taverna->taverna_tempo_de_profissao($idade_taverneiro),
			'garcon_personalidade_titulo' => '',
			'garcon_personalidade' => '',
			'personalidade_titulo' => $language->TAVERN_PERSONALITY_TITLE,
			'taverneiro_personalidade' => $taverna->taverna_personalidade_taverneiro(rand(2,4)),
			'aparencia_titulo' => $language->TAVERN_APPEARANCE_TITLE,
			'taverneiro_aparencia' => $taverna->taverna_aparencia_taverneiro(rand(2,3)),
			'comidas_titulo' => $language->TAVERN_SOME_FOOD_TITLE,
	...
	function bar(){
		...
		$taverna_medieval = [
			...
			'taverneiro_nome' => $language->TAVERN_OWNER . $taverna->taverna_nome_taverneiro(),
			'taverneiro_raca' => '',
			'taverneiro_idade' => $language->TAVERN_WITH . rand(22,100) . $language->TAVERN_YEARS,
			...
			'atracao_titulo' => $language->TAVERN_AN_ATTRACTION_TITLE,
			'taverna_atracao' => $taverna->taverna_artista() . $language->TAVERN_PLAYING_TITLE . $taverna->taverna_atracao_conteporanea(1)
		];
		return $taverna_medieval;
	...
	function restaurante(){
		...
		$taverna_medieval = [
			...
			'taverneiro_idade' => $language->TAVERN_WITH . rand(22,100) . $language->TAVERN_YEARS,
			'taverneiro_profissao' => '',
			'garcon_personalidade_titulo' => $language->TAVERN_GARCON_PERSONALITY_TITLE,
			...
			'taverna_porcoes' => $taverna->taverna_porcoes(rand(2,6)),
			'taverna_petiscos' => '',
			'pratos_titulo' => $language->TAVERN_PLATES_TITLE,
			'taverna_pratos' => $taverna->taverna_pratos(rand(3,8)),
			'sobremesa_titulo' => $language->TAVERN_DESSERT_TITLE,
