...
<?php
	...
	class Pericias_Controller extends Controller_Lib {
		...
		function index(){
			...
			$pericias_model = new Pericias_Model;
			...
			$pericias = $pericias_model->select('pericias');
		...
		function salvar(){
			...
			if (count($_REQUEST) < 4) {
				...
				if ($pericias->insert('pericias', $campos, $valores)) {
					$time_line = new Timeline_Model;
					$id_pericias = $pericias->maxId('pericias')[0];

					$time_line->insert(
					'timeline',
					['id_cadastro','lingua','titulo','exibir','tipo','dono','sistema','descricao'],
					[$id_pericias->id,$_REQUEST['lingua'], $_REQUEST['nome'], $exibir, 'pericias', $_REQUEST['dono'], $_REQUEST['sistema'], $_REQUEST['descricao']]);
					...
					header("Location: {$this->visualizar_path}{$id_pericias->id}");
				} else {
		...
		function visualizar($params=''){
			...
			$pericias_view = $pericias->direct_instruction("SELECT a.*, {$usuario_atributos} FROM pericias a, usuarios u WHERE a.id = {$params} and u.login = a.dono", 'pericias e usuarios')[0];
		...
		function editar($params){
			$pericias = new Pericias_Model();
			$pericias_view = $pericias->direct_instruction("SELECT * FROM pericias WHERE id={$params} AND dono='{$_SESSION['login']}'",'pericias');

			if (is_numeric($params) and count($pericias_view) == 1) {
				$pericias = (new Pericias_Model())->select('pericias','*',[ ['id','=',$params] ]);
				if (count($pericias) == 0) {
		...
		function atualizar(){
			...
			header('Location: ' . URL_BASE . 'erro/codigo/404');
				...
				$pericias = new Pericias_Model();
				$pericias_view = $pericias->direct_instruction("SELECT * FROM pericias WHERE id={$_REQUEST['id']} AND dono='{$_SESSION['login']}'",'armas');
				if (count($pericias_view) == 1) {
					if( preg_match( '@<img(.*?)/>@s', $_REQUEST['descricao'], $matches ) ){
						$exibir = $matches[0];
					} else {
						$exibir = '';
					...
					if ($pericias->update('pericias', $campos, $valores, 'id', $_REQUEST['id'])) {
						...
						$insrucao = "
							...
							WHERE
								id_cadastro = {$_REQUEST['id']} AND tipo = 'pericias'
						";

						$time_line->direct_instruction($insrucao,'timeline');

						header('Location: ' . URL_BASE . 'pericias/visualizar/'.$_REQUEST['id']);
					} else {
		...
		function deletar($params = ''){
			...
			if ($params == '' || !is_numeric($params) || (count($pericias_view) == 0)) {
				header('Location:' . URL_BASE. 'pericias');
			} else {
				if ($pericias->delete('pericias', ['id','=', $params])){
					$time_line = new Timeline_Model;
					$time_line->delete('timeline', [ ['id_cadastro', '=', $params], ['tipo', '=', 'pericias'] ], 'AND');
					header('Location: ' . URL_BASE . 'pericias?status=deletado!');
				} else {
					header('Location: ' . URL_BASE . 'erro/codigo/003');
				}
			}
